...
def plot_strategy_with_signals(df):
    ...
    df['Signal_s']= df['Signal'].shift(1).fillna(1)
    leverage_periods = df[df["Signal_s"]== 1]
    safe_periods = df[df["Signal_s"] == -1]
...
def plot_strategy_with_signals2(df):
    plt.figure(figsize=(14, 8))
    ...
    # 3. Mark Signal Changes using Signal_s
    signal_changes = df[df['Signal_s'].diff().ne(0).fillna(False)]
    for date in signal_changes.index:
        ax1.axvline(x=date, color='black', linestyle='--', alpha=0.5, linewidth=0.7)
        nav = df.loc[date, "Strategy_NAV"]
        new_position = df.loc[date, "Signal_s"]  # Get actual executed position
        ax1.plot(date, nav, 'o', markersize=6,
                markerfacecolor='red' if new_position == -1 else 'green',
                markeredgecolor='black')
    ...
    legend_elements = [
        Patch(facecolor='lightgreen', alpha=0.3, label='Leveraged (TQQQ/QLD)'),
        Patch(facecolor='lightcoral', alpha=0.3, label=f'Safe Asset'),
        Line2D([0], [0], color='black', linestyle='--', label='Execution Point'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='green', 
              markersize=8, label='Enter Leverage'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='red', 
              markersize=8, label='Enter Safe')
    ]
    ax1.legend(handles=legend_elements, loc='upper left')
...
def print_holding_periods(df):
    ...
    for group in df['Regime_Group'].unique():
        ...
        cumulative_return = (1 + df_group['Strategy_Return']).prod() - 1
        
        periods.append({
            'Position': 'Leverage' if period['Signal_s'] == 1 else 'Safe',
            'Start Date': start.strftime('%Y-%m-%d'),
            'End Date': end.strftime('%Y-%m-%d'),
            'Duration (Days)': duration,
            'VIX Start': round(period['^VIX'], 1),
            'VIX End': round(df.loc[end, '^VIX'], 1),
            'Gain (%)': round(cumulative_return * 100, 1)  # New gain column
        })
