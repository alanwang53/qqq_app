...
def download_data(START_DATE,END_DATE):
    ...
    if DATA_PATH.exists():
        try:
            ...
            shrunk_df = df.loc[START_DATE:END_DATE]
            return shrunk_df
...
def add_today_row(df):
    # Check if the DataFrame is empty
    if df.empty:
        # Create a new DataFrame with today's date and NaN values
        today = pd.Timestamp.today().normalize()
        new_data = {col: np.nan for col in df.columns}
        new_df = pd.DataFrame(new_data, index=[today])
        return new_df
    else:
        # Get the last index date and today's date, normalized to remove time
        last_date = df.index[-1].normalize()
        today = pd.Timestamp.today().normalize()
        
        # Check if the last date is before today
        if last_date < today:
            # Create a new row with today's date and NaN values
            new_data = {col: np.nan for col in df.columns}
            new_row = pd.DataFrame(new_data, index=[today])
